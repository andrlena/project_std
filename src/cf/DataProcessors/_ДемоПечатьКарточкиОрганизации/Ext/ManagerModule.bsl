...
Функция СведенияОбОрганизациях(СписокОрганизаций)
	...
	КонтактнаяИнформация.Сортировать("Тип, Вид, Представление");
	...
	Для Каждого Организация Из СписокОрганизаций Цикл
		...
		_ДемоЛокализация.ПриПолученииСведенийОбОрганизации(Организация, СвойстваОрганизации, Дата, ОсновныеСведения);
		...
		Для Каждого СтрокаКИ Из КонтактнаяИнформацияПоОрганизации Цикл
			Если СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
				...
				Если ЗначениеЗаполнено(НомерЧасовойЗоны) Тогда
					ПредставлениеАдреса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1. %2 Часовая зона: %3.'"),
						СтрокаКИ.ПредставлениеКонтактнойИнформации, Символы.ПС, НомерЧасовойЗоны);
					СтрокаКИ.ПредставлениеКонтактнойИнформации = ПредставлениеАдреса;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
